...
sep_old <- 
  ...
  filter(u_grad > 10) %>% group_by(title) %>% summarise(total_enrollment = sum(u_grad), .groups = "drop")
...
- Define what we want to know 
  - why the drop in course enrollment occurred from sept. 1 to sept. 22
  - we want to know the reason for this drop, recognizing we are in a pandemic
  - Be thoughtful in using and cleaning data


After we have a model, Temperance suggests we should:

- Recognize our model cannot explain all of the reasons people dropped
  - each individual may have different reasons, not one reason!
...
```{r}
sep_combine <-
  full_join(sep_old, sep_new, by = ("title")) %>% 
